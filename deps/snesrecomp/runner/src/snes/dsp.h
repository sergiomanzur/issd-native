...
struct Dsp {
  ...
  void *shadow;
  // Host PCM continuity only, outside the serialized guest/DSP region.
  // A full FIFO drops source samples; join its next accepted sample to the
  // last queued sample at that exact boundary, without touching guest time.
  uint32_t outputRecoveryRemaining;
  int16_t outputRecoveryL;
  int16_t outputRecoveryR;
  // mirror ram
  uint8_t ram[0x80];
  // 8 channels
