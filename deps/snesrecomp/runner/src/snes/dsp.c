...
void dsp_reset(Dsp* dsp) {
  dsp->outputRecoveryRemaining = 0;
  dsp->outputRecoveryL = 0;
  dsp->outputRecoveryR = 0;
...
void dsp_saveload(Dsp *dsp, SaveLoadInfo *sli) {
  uint32_t previousWrite = dsp->sampleWrite;
  uint32_t previousRead = dsp->sampleRead;
  sli->func(sli, &dsp->ram, sizeof(Dsp) - offsetof(Dsp, ram));
  if (dsp->sampleWrite != previousWrite || dsp->sampleRead != previousRead)
    dsp->outputRecoveryRemaining = 0;
}
...
void dsp_cycle(Dsp* dsp) {
  ...
  if (dropped) {
    /* The missing segment belongs after the last queued pair, not at the
     * consumer's current read position (which can be 256 ms earlier). Keep
     * the existing queue intact and ramp only samples appended after it. */
    uint32_t last = (dsp->sampleWrite - 1) & (DSP_SAMPLE_RING - 1);
    dsp->outputRecoveryL = dsp->sampleBuffer[last * 2];
    dsp->outputRecoveryR = dsp->sampleBuffer[last * 2 + 1];
    dsp->outputRecoveryRemaining = 64;
  } else {
    uint32_t w = dsp->sampleWrite & (DSP_SAMPLE_RING - 1);
    int outL = totalL, outR = totalR;
    if (dsp->outputRecoveryRemaining != 0) {
      /* Two milliseconds at the native rate removes the hard waveform seam.
       * Raw DSP output and trace remain untouched; only host PCM is joined. */
      uint32_t oldWeight = --dsp->outputRecoveryRemaining;
      outL = (dsp->outputRecoveryL * (int)oldWeight + totalL * (64 - (int)oldWeight)) / 64;
      outR = (dsp->outputRecoveryR * (int)oldWeight + totalR * (64 - (int)oldWeight)) / 64;
    }
    dsp->sampleBuffer[w * 2] = outL;
    dsp->sampleBuffer[w * 2 + 1] = outR;
    dsp->sampleWrite++;
  }
