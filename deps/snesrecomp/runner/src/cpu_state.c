...
static void cpu_diag_nmi_busy_write(CpuState *cpu, uint8 bank, uint16 addr,
                                    uint16 value, int width) {
    static int initialized;
    static int enabled;
    static int first_frame;
    if (!initialized) {
        const char *watch = getenv("SNESRECOMP_NMI_BUSY_WATCH");
        enabled = watch && *watch;
        first_frame = enabled ? (int)strtol(watch, NULL, 0) : 0;
        initialized = 1;
    }
    if (!enabled || (bank & 0x7Fu) > 0x3Fu || addr > 0x1FFFu)
        return;
    if (!((width == 1 && addr == 0x003Cu) ||
          (width == 2 && addr <= 0x003Cu &&
           (uint16)(addr + 1u) >= 0x003Cu)))
        return;
    {
        extern int snes_frame_counter;
        extern const char *g_last_recomp_func;
        if (snes_frame_counter < first_frame)
            return;
        fprintf(stderr,
                "[nmi-busy-write] frame=%d width=%d addr=$%04X "
                "value=$%04X S=$%04X PB=$%02X func=%s interp_pc=$%06X\n",
                snes_frame_counter, width, addr, value, cpu->S, cpu->PB,
                g_last_recomp_func ? g_last_recomp_func : "<none>",
                g_interp816_cur_pc & 0xFFFFFFu);
        fflush(stderr);
    }
}

void cpu_write8(CpuState *cpu, uint8 bank, uint16 addr, uint8 v) {
    cpu_diag_nmi_busy_write(cpu, bank, addr, v, 1);
    /* Diagnostic tripwire for native-recompiler stack corruption.  A native
     * mode 65816 stack starts in page $01 and grows downward.  Once a push
     * reaches direct page, the next bytes overwrite live game state before
     * the frame-level balance checks can observe the transient underflow.
     * Keep this opt-in so normal direct-page writes remain cost-free. */
    if (bank == 0 && addr == cpu->S && cpu->S < 0x0100u &&
        getenv("SNESRECOMP_STACK_TRIPWIRE")) {
        static int reported;
        if (!reported) {
            extern int snes_frame_counter;
            extern const char *g_last_recomp_func;
            reported = 1;
            fprintf(stderr,
                    "[stack-tripwire] frame=%d S=$%04X value=$%02X "
                    "func=%s interp_pc=$%06X\n",
                    snes_frame_counter, cpu->S, v,
                    g_last_recomp_func ? g_last_recomp_func : "<none>",
                    g_interp816_cur_pc & 0xFFFFFFu);
            RecompStackDump();
            fflush(stderr);
...
void cpu_write16(CpuState *cpu, uint8 bank, uint16 addr, uint16 v) {
    cpu_diag_nmi_busy_write(cpu, bank, addr, v, 2);
    if (g_snes && g_snes->cart)
        cart_note_cpu_bus(g_snes->cart, bank, addr);
