...
// re-render of a guest audio engine (here the S-DSP's BRR voices, free of the
// hardware Gaussian interpolation and 16-bit intermediate truncation) runs
// ALONGSIDE the canon hardware mix, never replacing it. This verifier decides,
// from the running canon stream, whether the shadow is proven enough to
// substitute, and reverts loudly the instant it stops matching:
//
...
//
// The canon (emulated S-DSP) output stays the verify oracle; the shadow is
// never ground truth. If this verifier is wrong, the worst case is "we keep
...
// used with permission. Engine-agnostic; reused across the recomp ecosystem.
...
#endif  // SNESRECOMP_AUDIO_SHADOW_H
